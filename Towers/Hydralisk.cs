    ...
    public class Hydralisk{
        ...
        public static UpgradeModel[]GetUpgrades(){
            return new UpgradeModel[]{
                new("Grooved Spines",250,0,new("HydraliskGroovedSpines"),0,1,0,"","Grooved Spines"),new("Frenzy",800,0,new("HydraliskFrenzy"),0,2,0,"","Frenzy")
            };
        }
        public static(TowerModel,TowerDetailsModel,TowerModel[],UpgradeModel[])GetTower(GameModel gameModel){
            ...
            return (GetT000(gameModel),HydraliskDetails,new[]{GetT000(gameModel),GetT100(gameModel),GetT200(gameModel)},GetUpgrades());
        }
        ...
        public static TowerModel GetT200(GameModel gameModel){
            ...
            ab.icon=new("HydraliskFrenzy");
            var aam=ab.behaviors[0].Cast<CreateEffectOnAbilityModel>();
            Hydralisk.behaviors.Add(aam);
            return Hydralisk;
        }
        ...
        public record ResourceLoader_Patch{
            ...
            public static void Postfix(SpriteReference reference,ref Image image){
                ...
                if(reference!=null&&reference.guidRef.Equals("HydraliskFrenzy")){
                    var b=Assets.LoadAsset("HydraliskFrenzy");
                    var text=b.Cast<Texture2D>();
                    image.canvasRenderer.SetTexture(text);
                    image.sprite=Sprite.Create(text,new(0,0,text.width,text.height),new());
                }
                if(reference!=null&&reference.guidRef.Equals("HydraliskGroovedSpines")){
                    var b=Assets.LoadAsset("HydraliskGroovedSpines");
                    var text=b.Cast<Texture2D>();
                    image.canvasRenderer.SetTexture(text);
                    image.sprite=Sprite.Create(text,new(0,0,text.width,text.height),new());
                }
        ...
        public static class WI{
            ...
            private static async Task RunAnimations(Weapon __instance){
                if(__instance.weaponModel.name.Contains("HydraliskSpine")){
                    ...
                    __instance.attack.tower.Node.Graphic.gameObject.GetComponentInParent<Animator>().Play("Armature Object|Armature ObjectAttack");
